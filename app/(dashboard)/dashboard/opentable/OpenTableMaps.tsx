...
export const MapsComponents = () => {
  const { data, isLoading, error } = useSWR(
    "/dashboard/opentable/api/attendance",
    fetcher
  );
  ...
  const convertToAsiaJakartaTimezone = (
    timestamp: string | undefined
  ): string | undefined => {
    if (!timestamp) return undefined;

    const asiaJakartaTimezoneDate = new Date(timestamp).toLocaleString(
      "en-US",
          ...
          <HoverCard.Dropdown>
            <div className="flex flex-col justify-center items-center text-center">
              <p className="font-bold">
                {attendance.profilekcontact} - {attendance.profilename}
              </p>
              <p>{convertToAsiaJakartaTimezone(attendance.checkInTime)}</p>
              <p className="flex my-2">
                Nomor HP - {attendance.profilephonenumber}
              </p>
              <Divider my={5} />
