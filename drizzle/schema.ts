import { pgTable, pgEnum, varchar, timestamp, text, integer, uniqueIndex, serial, index, doublePrecision, pgView } from "drizzle-orm/pg-core"
...
export const userAttendance = pgTable("userAttendance", {
	id: serial("id").primaryKey().notNull(),
	checkInTime: timestamp("checkInTime", { precision: 6, withTimezone: true, mode: 'string' }).notNull(),
	checkOutTime: timestamp("checkOutTime", { precision: 6, withTimezone: true, mode: 'string' }),
...
export const visitorReport = pgTable("visitorReport", {
	...
	createdAt: timestamp("createdAt", { precision: 3, mode: 'date', withTimezone: true }).defaultNow().notNull(),
	userId: integer("userId"),
},
(table) => {
	return {
